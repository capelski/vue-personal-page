...
const getGoogleMaps = () => {
    return window.googleMaps
        ...
        : loadGoogleMapsApi({ key: process.env.MAPS_API_KEY }).then(_googleMaps => {
        ...
        meta: [
            {
                name: 'description',
                content:
                    'The list of places I have proudly travelled to along with an iteractive map. In constant expansion :D'
            }
        ]
    },
    data() {
            ...
            for (let i = 0; i < this.markers.length; i++) {
                ...
                googleMaps.event.addListener(marker, 'click', () => {
                    infowindow.setContent(
                        `${markerData.label.city}, ${markerData.label.date}, ${markerData.label.duration}`
                    );
